...
def test_get_weather_intervals_for_similar_sites():
    ...
    df_collated, df_shorter_intervals = eeweather.get_weather_intervals_for_similar_sites(df)
    for i in df_shorter_intervals.iterrows():
        shorter_interval = i[-1]['end_date'] - i[-1]['start_date']
        larger_interval = df_collated[df_collated.index == i[-1]['matching_index']]['end_date'] - \
